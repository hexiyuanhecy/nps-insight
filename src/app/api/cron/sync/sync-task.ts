...
async function sendNotification(syncedCount: number): Promise<boolean> {
  ...
  try {
    ...
    for (const r of records) {
      ...
      const reviewNeeded = String(fields[FEEDBACK_FIELDS.REVIEW_NEEDED] || '').toLowerCase() === '是';
      const needLog = String(fields[FEEDBACK_FIELDS.NEED_LOG_CHECK] || '').toLowerCase() === '是';
    ...
    const topIssues = sortedTag3.map(([tag3Name, count]) => ({
      tag1: '', tag2: '', tag3: tag3Name, count, pct: scoreCount > 0 ? Math.round((count / scoreCount) * 100) : 0
    }));
    ...
    const avgScore = scoreCount > 0 ? (scoreCount / total).toFixed(1) : '0';
...
async function enrichTenantInfo(feedbacks: Array<{ tenantId?: string; tenantName?: string }>): Promise<Map<string, string>> {
  const tenantIds = new Set<string>();
  for (const f of feedbacks) { if (f.tenantId) tenantIds.add(f.tenantId); }
...
async function generateWeeklyDoc(): Promise<boolean> {
  try {
    ...
    if (result.success && result.documentUrl) {
      ...
      console.error(`[Cron] 周报文档生成失败: ${result.error}`);
      return false;
    }
  } catch (error) {
...
async function runSyncWithMockData(): Promise<SyncResult> {
  ...
  try {
    ...
    const { feedbackList } = batchGenFeedback(mockCount);
    console.log(`[Mock] 生成 ${feedbackList.length} 条反馈`);
...
const PLATFORM_CONTENT_MAP: Record<string, string[]> = {
  '打卡小程序': ['打卡定位失败', '点击打卡按钮没反应', '外勤打卡提交后页面报错', '希望支持批量打卡', '建议增加一键补卡功能', 'test', '测试数据'],
  '休假小程序': ['假期余额计算错误', '休假申请提交后页面报错', '审批流程卡住', '希望自定义审批模板', 'test', '测试数据'],
};
